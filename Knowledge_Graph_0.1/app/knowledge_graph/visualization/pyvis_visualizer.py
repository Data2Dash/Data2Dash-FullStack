...
(function () {
  ...
  // ------- setup node click messaging to parent iframe -------
  function setupNodeClick() {
    if (!network) { setTimeout(setupNodeClick, 300); return; }
    network.on("doubleClick", function(params) {
      if (params.nodes.length > 0) {
        const nodeId = params.nodes[0];
        const label = getLabel(nodeId);
        window.parent.postMessage({ type: 'kg_node_click', node: label }, '*');
      }
    });
  }
