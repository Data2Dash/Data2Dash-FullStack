...
class QueryExpander:
    ...
    def expand(self, query: str, prf_terms: List[str] = None) -> Dict[str, Any]:
        ...
        # Hard cap: never expand a query longer than 300 chars.
        # Queries this long are almost certainly raw paper abstracts or context
        # strings that should not be sent to the LLM or used as ArXiv queries.
        _MAX_EXPAND_CHARS = 300
        if len(query) > _MAX_EXPAND_CHARS:
            truncated = query[:_MAX_EXPAND_CHARS]
            last_space = truncated.rfind(" ")
            query = truncated[:last_space].strip() if last_space > 0 else truncated
            print(
                f"[QueryExpander] Query truncated to {len(query)} chars before expansion",
                file=sys.stderr,
            )
