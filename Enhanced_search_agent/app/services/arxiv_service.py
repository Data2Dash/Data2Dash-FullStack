...
class ArxivService:
    ...
    def __init__(self):
        ...
        self.client = arxiv.Client(
            page_size=10,
            delay_seconds=3.0,   # library-level protection against rapid successive calls
    ...
    def search_papers(
        ...
        # ── Hard cap: ArXiv returns HTTP 500 for query strings > ~500 chars
        # when URL-encoded.  Truncate to the last whole word within 200 chars.
        _MAX_QUERY_CHARS = 200
        if len(query) > _MAX_QUERY_CHARS:
            truncated = query[:_MAX_QUERY_CHARS]
            last_space = truncated.rfind(" ")
            query = truncated[:last_space].strip() if last_space > 0 else truncated
            print(
                f"[ArxivService] Query truncated to {len(query)} chars to avoid HTTP 500",
                file=sys.stderr,
            )
        ...
        for attempt in range(self._MAX_RETRIES):
            try:
                ...
                err_str = str(e)
                is_rate_limit = "429" in err_str or "Too Many Requests" in err_str or "HTTP 429" in err_str
                if is_rate_limit:
                    self._set_cooldown(120.0)  # back off for 2 minutes on 429
